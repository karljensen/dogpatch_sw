...
typedef struct sources_t {
    ...
    volatile uint32_t listen[3]; // this isn't 3 choices, it is one choice 12 char long
    volatile uint32_t action[3]; // this isn't 3 choices, it is one choice 12 char long
} sources_t;
...
typedef struct dogpatch_image_info_t {
   ...
   volatile uint32_t risk_expiry_ms; // max 16 bits
   ...
   volatile uint32_t reserved3[77];
...
extern const char* RISK_CODES[18];
